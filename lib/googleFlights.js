...
async function fetchText(url, init = {}) {
  ...
  const blocked = res.status === 429 || /\/sorry\//.test(res.url);
  if (blocked) throw new Error('Google temporarily rate-limited this server ("unusual traffic"); will retry on the next check');
  if (!res.ok) throw new Error(`Google Flights responded with HTTP ${res.status}`);
  return res.text();
}

function parseSearchHtml(html, currency) {
  if (/unusual traffic from your computer network/i.test(html)) {
    throw new Error('Google temporarily rate-limited this server ("unusual traffic"); will retry on the next check');
  }
  const data = extractDataBlock(html, 'ds:1');
  if (!data) throw new Error('Google Flights returned no flight data (page layout changed?)');
  if (currency && !html.includes(`"${currency}"`)) throw new Error(`Google Flights did not return prices in ${currency}`);
  // data[5] = price insights: [level, [null, lowest], ..., [null, typicalLow], [null, typicalHigh], ...]
  const typicalLow = data[5]?.[4]?.[1];
  const typicalHigh = data[5]?.[5]?.[1];
  return {
    itineraries: [2, 3].flatMap(k => data?.[k]?.[0] || []).map(parseItinerary),
    baggageLinks: Object.fromEntries((data[11] || []).map(([code, , url]) => [code, url])),
    typicalRange: typeof typicalLow === 'number' && typeof typicalHigh === 'number' && typicalLow < typicalHigh
      ? [typicalLow, typicalHigh] : null
  };
}

/** Runs a Google Flights search page and returns every itinerary it lists. */
async function searchPage(config, outbound = null) {
  return parseSearchHtml(await fetchText(googleSearchUrl(config, outbound)), config.currency || 'CAD');
}

/**
 * Reference fares for one airline on its own network (e.g. Air India Toronto <-> Delhi), used to
 * explain prices when the airline publishes no fare for the full itinerary.
 */
export async function airlineReferenceFares(config, { code, name, hub }) {
  const run = async (legs, trip) => {
    const url = `${BASE}/search?tfs=${buildTfs({ legs, trip })}&${localeParams(config)}`;
    const { itineraries, typicalRange } = parseSearchHtml(await fetchText(url), config.currency || 'CAD');
    const priced = itineraries.filter(i => i.price != null && i.legs.every(l => l.airline === code))
      .sort((a, b) => a.price - b.price);
    const best = priced[0];
    return best ? {
      price: best.price,
      typicalRange,
      flights: best.legs.map(l => `${l.airline} ${l.flightNumber}`),
      url
    } : null;
  };
  const out = { date: config.dateFrom, from: config.origin, to: hub, airlines: [code] };
  const back = { date: config.dateTo, from: hub, to: config.origin, airlines: [code] };
  const [roundTrip, oneWay] = await Promise.all([run([out, back], 1), run([out], 2)]);
  return { code, name, hub, roundTrip, oneWay };
}
...
const MIN_PLAUSIBLE_PRICE = 300;
const MAX_PLAUSIBLE_PRICE = 20000;

/**
 * Sanity checks so wrong data never reaches the dashboard. Returns a reason string if the trip
 * must be dropped, or null if it is consistent.
 */
function validateTrip(config, { outbound, inbound, price, anyFare, googlePrice }) {
  if (outbound.from !== config.origin || outbound.to !== config.destination || outbound.departDate !== config.dateFrom) {
    return `outbound ${outbound.from}-${outbound.to} ${outbound.departDate} does not match the search`;
  }
  if (inbound.from !== config.destination || inbound.to !== config.origin || inbound.departDate !== config.dateTo) {
    return `return ${inbound.from}-${inbound.to} ${inbound.departDate} does not match the search`;
  }
  if (!Number.isInteger(price) || price < MIN_PLAUSIBLE_PRICE || price > MAX_PLAUSIBLE_PRICE) {
    return `implausible price ${price}`;
  }
  // Two independent Google sources must agree: the results list price for this exact trip vs
  // the cheapest seller on the booking page.
  if (anyFare && googlePrice && Math.abs(anyFare.price - googlePrice) / googlePrice > 0.2) {
    return `booking price ${anyFare.price} disagrees with listed price ${googlePrice}`;
  }
  return null;
}
...
export async function findFlights(config) {
  ...
  const { itineraries: outbounds, baggageLinks, typicalRange } = await searchPage(config);
  ...
  const trips = await mapLimit(pricedOut, 3, async (outbound) => {
    ...
    // Cheapest fare for this exact trip regardless of bags (e.g. Air Canada "Standard" with 1 bag)
    const anyFare = sellers.find(o => !o.separateTickets) || null;
    const problem = validateTrip(config, { outbound, inbound, price: cheapest.price, anyFare, googlePrice: inbound.price });
    if (problem) {
      console.warn(`[validate] Dropped ${outbound.key}/${inbound.key}: ${problem}`);
    ...
    return {
      ...
      cheapestAnyBags: anyFare && anyFare.price < cheapest.price ? summarize(anyFare) : null,
      googleListedPrice: inbound.price,
  ...
  const options = trips.filter(t => t?.id).sort((a, b) => a.price - b.price);
  const errors = trips.filter(t => t?.error).map(t => t.error);
  const rejected = trips.filter(t => t?.rejected).map(t => t.rejected);

  return {
    fetchedAt: new Date().toISOString(),
    searchUrl: googleSearchUrl(config),
    typicalRange,
    // true when some itineraries could not be priced this run (e.g. rate-limited midway)
